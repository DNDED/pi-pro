...
import type { PiProConfig } from "@pi/provider";
...
describe("formatConfigDisplay", () => {
  it("shows the provider line", () => {
    const out = formatConfigDisplay({ provider: "opencode-go", model: "minimax-m3" } as PiProConfig, undefined);
  ...
  it("masks an apiKey showing only the first 8 chars + ellipsis", () => {
    const out = formatConfigDisplay(
      { provider: "opencode-go", model: "minimax-m3" } as PiProConfig,
      "sk-abcde1234567890",
    );
...
describe("config orchestrator", () => {
  ...
  it("with set <key> <value> and a valid model, writes model to config and prints confirmation", async () => {
    ...
    const raw = await readFile(join(home, ".pi", "agent", "pi-pro-config.json"), "utf8");
  ...
  it("with set apiKey <key>, writes auth to auth.json", async () => {
    ...
    const raw = await readFile(join(home, ".pi", "agent", "pi-pro-auth.json"), "utf8");
