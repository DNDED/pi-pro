...
import type { SessionEvent } from "@promyra/tasks";

let dir: string;
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), "replay-test-"));
  process.env.PROMYRA_HOME_OVERRIDE = dir;
});
afterEach(async () => {
...
describe("replay orchestrator (uses process.cwd())", () => {
  it("prints 'No session log' for a task with no events in an empty tmpdir", async () => {
    ...
    // *is* the test's cwd (or at least there is no .promyra/sessions for this task).
