# Pi-Pro: An Improved Pi Coding Agent — Design
...
Pi-Pro combines both. It ships the skill discipline and subagent architecture on top of pi's existing tool harness, plus a state machine and checkpointing that make long agentic tasks resumable and debuggable. The TUI gets OpenCode's visual feel as a final polish layer.
...
- GUI configuration. Config via `~/.pi/agent/AGENTS.md`.
...
- **`@pi/skill-bundle`** — pure data + a tiny loader. Contains the curated skills directory and a `postinstall` script that copies them into `~/.pi/agent/skills/pi-pro/`. No runtime code. The default system prompt is a `prompt.md` file in this package, applied via pi's existing `systemPromptFile` config.
