...
This directory is the durable memory for the **promyra** project (the TS fork of pi-mono that lives at `/home/trader/Developer/pi-pro`).
...
├── projects/
│   └── promyra.md               # project note: architecture, releases, files of interest
├── daily/
...
1. `memory/user.md`
2. `memory/projects/promyra.md`
3. `memory/decisions/v0.6.0.md` (and `v0.5.0.md`)
...
1. Touched project? → update `projects/promyra.md`
...
The Obsidian vault at `/home/trader/Documents/Obsidian Vault/` is the global memory for other projects (leadops, argent, etc.). Promyra is intentionally NOT in the Obsidian vault — it lives in-repo only. Sid can manually mirror if needed (e.g. for cross-project search), but there's no automated sync.

## Origin

This memory system was created on 2026-06-11 during the v0.6.0 design session. Before this date, promyra memory was stored in the Obsidian vault under `Projects/promyra.md`, `Daily/2026-06-11.md`, and `Sessions/OpenCode/`. Those entries are preserved in the vault for historical reference.
